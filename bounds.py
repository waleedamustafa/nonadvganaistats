...
from architectures import output_transform
...
class PBBobj():
    ...
    def compute_empirical_cert(self,sum_error,n_points, alpha,raw):
        cert_2 = sum_error/n_points 
        cert_3 = np.sqrt(2*alpha*(1-alpha)*np.log(1.0/raw)/n_points)
        cert_4 = np.log(1/raw)/(3*n_points)
        cert = (cert_2+alpha-cert_3+cert_4)
        # print(cert_2,cert)
        return cert if cert <=1.0 else 1.0
    ...
    def compute_losses(self, net, data, target, clamping=True):
        ...
        outputs = output_transform(outputs,clamping=clamping,pmin=self.pmin)
    ...
    def compute_01_risk(self,error_01,kl):
        empirical_risk_01 = inv_kl(
            error_01, np.log(2/self.delta_test)/self.mc_samples)
        risk_01 = inv_kl(empirical_risk_01, (kl + np.log((2 *
                                                             np.sqrt(self.n_bound))/self.delta_test))/self.n_bound)
...
def inv_kl(qs, ks):
    ...
    return 1.0 if is_naive_bound else qd
